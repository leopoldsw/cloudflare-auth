...
async function verifyTroubleshootingDocs() {
  ...
  for (const [problem, fix] of [
    ...
    ["Setup did not converge", "cf-auth setup --report --env production"],
    ["Setup did not converge", "docs/manual-steps.md"],
    ["workers.dev subdomain not registered", "cf-auth setup --env production"],
    ["Email Sending requires Workers Paid", "docs/manual-steps.md"],
